...
export default function LeavePageDashboard() {
  ...
  const fetchLeaveRequests = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        "https://david-backend-production.up.railway.app/admin/fetch-leave-requests",
        {
          method: "GET",
          headers: {
      ...
      data.data.forEach((request: { id: string; status: string }) => {
        console.log(
          `Request ID: ${request.id}, Status: ${request.status}, Type of status: ${typeof request.status}`,
        );
      });
      ...
      const pendingRequests = data.data.filter(
        (request: { status: number; id: string }) => {
          ...
          console.log(
            `Request ${request.id}: status=${request.status}, matches status 1? ${hasStatus1}`,
          );
          return hasStatus1;
        },
      );
  ...
  const updateLeaveRequestStatus = useCallback(
    async (id: number, status: number) => {
      try {
        console.log(
          `Attempting to update request - ID: ${id}, Status: ${status}`,
        );
        if (!id) {
        ...
        <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6 max-w-7xl mx-auto w-full">
          <div className="flex justify-between items-center mb-4">
            <h2 className="text-lg font-semibold">
              Active Leave Requests ({leaveRequests.length})
            </h2>
            <Button
              variant="outline"
              size="sm"
          ...
          {loading && (
            <p className="text-blue-500 mb-4">Loading leave requests...</p>
          )}
